...
function GraphEditorContent() {
  ...
  const currentGraph = useSelector(
    (state: RootState) => state.graphs.currentGraph
  );
  ...
  useEffect(() => {
    if (!graphId || graphId === "undefined") {
      ...
      setError(
        "No valid graph ID provided. Please select a graph from the graph manager."
      );
      setIsLoading(false);
    ...
    if (graph && graph.graph_data) {
      ...
      console.log(
        "[GraphEditor] Graph not found in store or incomplete, attempting to fetch..."
      );
